...
| `auth_token`         | String  | اختیاری    | توکن احراز هویت مشترک. تنها کلاینت‌هایی حق ارتباط دارند که این توکن را داشته باشند.                                                         |
...
::: tip آموزش جامع ساخت و مدیریت ورودی‌ها
ققنوس از انواع مختلفی از ورودی‌ها مانند SOCKS5، Shadowsocks و تونل SSH پشتیبانی می‌کند. اگر نیاز به توضیحات بیشتر و آشنایی با نحوه کار هر ورودی (و رمزنگاری‌های آن‌ها) دارید، حتماً صفحه **[مدیریت خروجی‌ها (Inbounds)](inbounds.md)** را مطالعه کنید!
:::

::: tip غیرفعال‌سازی ورودی
اگر می‌خواهید یک ورودی را غیرفعال کنید، کافیست علامت `#` را در ابتدای خطوط آن قرار دهید تا کامنت شوند.
:::

::: warning نکته مهم در مورد پشتیبانی سرور
دقت کنید تنها در صورتی inbound تعریف شده کار می‌کند که سرور از قابلیت مربوطه پشتیبانی کند (مثلاً `enable_shadowsocks = true` در کانفیگ سرور).
:::
...
| `-gen-token` | هر دو  | یک توکن تصادفی امن برای استفاده در `auth_token` تولید می‌کند.                            |
