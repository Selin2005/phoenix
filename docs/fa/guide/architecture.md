...
ققنوس از کتابخانه [`github.com/refraction-networking/utls`](https://github.com/refraction-networking/utls) استفاده می‌کند تا `ClientHello` را **دقیقاً شبیه مرورگرهای واقعی** بسازد.
...
1. کلاینت یک اتصال TCP معمولی می‌سازد
2. `utls.UClient` با `HelloID = HelloChrome_Auto` آغاز می‌شود
3. پیام `ClientHello` **دقیقاً** شبیه Chrome 120 ساخته می‌شود (همان cipher suites، همان extensions، همان ترتیب)
4. سرور TLS handshake معمولی انجام می‌دهد
5. هیچ تفاوتی از دید DPI بین این ترافیک و Chrome واقعی وجود ندارد
